    ...
    sql = f'''SELECT
              ...
              FROM (
                  ...
                  FROM
                      category
                  WHERE
                      category_id={category_id}) AS category
...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    sql = f'''SELECT
                  category.name category, COUNT(film_category.film_id)
              FROM (
                  SELECT
                      category_id, name
                  FROM
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    sql = f'''SELECT
                  customer.first_name first_name, customer.last_name last_name, SUM(film.length) sum
              FROM
                  customer
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    sql = f'''SELECT
                  category.name category, AVG(film.length), SUM(film.length), MIN(film.length), MAX(film.length)
              FROM (
                  SELECT
                      category_id, name
                  FROM
